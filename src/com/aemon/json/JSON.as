				...
				switch (stateStack[stateStack.length - 1]) {
					...
					case PARSE_STATE_ARRAY_START: {
						...
						* than state_start */
						...
						switch (tok.type) {
							...
							*/
							...
							case Token.TYPE_RBRACKET: {
								if (stateStack[stateStack.length - 1] == PARSE_STATE_ARRAY_START)
								{
									trace("end_array");
									// TODO
									//if (hand->callbacks &&
						...
						if(aroundAgain) break;

						/* got a value. transition depends on the state we're in. */
						var s:int = stateStack[stateStack.length - 1];
						if (s == PARSE_STATE_START) {
							stateStack.push(PARSE_STATE_PARSE_COMPLETE);
						} else if (s == PARSE_STATE_MAP_NEED_VAL) {
							stateStack.push(PARSE_STATE_MAP_GOT_VAL);
						} else {
							stateStack.push(PARSE_STATE_ARRAY_GOT_VAL);
						}
					...
					case PARSE_STATE_MAP_NEED_KEY: {
						...
						* a comma, and a string key _must_ follow */
						...
						switch (tok.type) {
							...
							//			bufLen));
							//}
							stateStack.push(PARSE_STATE_MAP_SEP);
							aroundAgain = true;
							break;

							case Token.TYPE_RBRACE:
							if (stateStack[stateStack.length - 1] == PARSE_STATE_MAP_START)
							{
								trace("end_map")
								//TODO
					...
					case PARSE_STATE_MAP_SEP: {
						...
						switch (tok.type) {
							case Token.TYPE_COLON:
							stateStack.push(PARSE_STATE_MAP_NEED_VAL);
							aroundAgain = true;
							break;
					...
					case PARSE_STATE_MAP_GOT_VAL: {
						...
						switch (tok.type) {
							...
							case Token.TYPE_COMMA:
							stateStack.push(PARSE_STATE_MAP_NEED_KEY);
							aroundAgain = true;
							break;
					...
					case PARSE_STATE_ARRAY_GOT_VAL: {
						...
						switch (tok.type) {
							...
							case Token.TYPE_COMMA:
							stateStack.push(PARSE_STATE_ARRAY_NEED_VAL);
							aroundAgain = true;
							break;
